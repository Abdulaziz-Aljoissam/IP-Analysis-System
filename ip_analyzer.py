...
def run_bulk_scan(use_api=False):
    ...
    with ThreadPoolExecutor(max_workers=30) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(targets), desc="    Local Scanning", bar_format="{l_bar}{bar:20}{r_bar}", colour='green'):
            res = future.result()
    ...
    if use_api:
        ...
        if unknowns:
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                list(tqdm(executor.map(lambda r: process_online(r, engine), unknowns), total=len(unknowns), desc="    API Processing", bar_format="{l_bar}{bar:20}{r_bar}", colour='cyan'))
